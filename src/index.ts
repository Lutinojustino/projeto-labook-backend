...
app.post("/users/signup", async (req: Request, res: Response) => {
  try {
    ...
    const [userId]: UsersDB[] | undefined[] = await db("users").where(
      "id",
      "=",
      `${id}`
    );
    ...
    const [userEmail]: UsersDB[] | undefined[] = await db("users").where(
      "email",
      "=",
      `${email}`
    );
...
app.post("/users/login", async (req: Request, res: Response) => {
  try {
    ...
    const [userExist]: UsersDB[] | undefined[] = await db("users").where(
      "email",
      "=",
      `${email}`
    );
...
app.put("/posts/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [postExist]: PostsDB[] | undefined[] = await db("posts").where(
      "id",
      "=",
      `${id}`
    );
